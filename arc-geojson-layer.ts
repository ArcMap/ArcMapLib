...
type ArcMapLike = HTMLElement & {
  ...
  getViewInstance?: () => Promise<MapView | null>;
  enableInfoWindow?: (enable: boolean) => void;
  _jsView?: MapView | null;
};
...
export class ArcGeojsonLayer extends LitElement {
  ...
  private viewClickHandle: RemoveHandle | null = null;
  private viewDoubleClickHandle: RemoveHandle | null = null;
  private viewPointerMoveHandle: RemoveHandle | null = null;
  private sketchCreateHandle: RemoveHandle | null = null;
  private sketchUpdateHandle: RemoveHandle | null = null;
  ...
  labelColor: string | number[] = [0, 0, 0, 255];
  ...
  private async initializeComponent(): Promise<void> {
    ...
    if (!this.view) {
      console.error('[arc-geojson-layer] ArcGIS view is null.');
      return;
    }
  ...
  private async resolveViewFromAncestor(): Promise<void> {
    ...
    if (!this.view) {
      console.error('[arc-geojson-layer] Could not resolve ArcGIS view from arc-map.');
      return;
    }
  ...
  private attachViewHandlers(): void {
    ...
    this.viewClickHandle = this.view.on('click', async (event: any) => {
    ...
    this.viewDoubleClickHandle = this.view.on('double-click', async (event: any) => {
      if (!this.view || !this.graphicsLayer) return;
    ...
    this.viewPointerMoveHandle = this.view.on('pointer-move', async (event: any) => {
      if (!this.view || !this.graphicsLayer) return;
      ...
      const hitGraphic = result?.graphic as Graphic | undefined;

      if (hitGraphic && this.hoveredGraphic !== hitGraphic) {
        this.hoveredGraphic = hitGraphic;
        this.dispatchEvent(
      ...
      if (!hitGraphic && this.hoveredGraphic) {
        const oldGraphic = this.hoveredGraphic;
        this.hoveredGraphic = null;
        this.dispatchEvent(
          new CustomEvent('layerMouseOut', {
            detail: {
              graphic: oldGraphic,
              attributes: oldGraphic.attributes ?? {},
            },
            bubbles: true,
            composed: true,
          })
  ...
  private attachSketchHandlers(): void {
    ...
    this.sketchCreateHandle = this.sketchVM.on('create', (event: any) => {
    ...
    this.sketchUpdateHandle = this.sketchVM.on('update', (event: any) => {
      if (event.state === 'start') {
        this.enableInfoPopupWindow(false);
      }

      if (event.state === 'complete') {
        const updatedGraphics = (event.graphics ?? []) as Graphic[];
        updatedGraphics.forEach((g) => this.decorateGraphic(g));
        ...
        this.dispatchEvent(
          new CustomEvent('userEditItemUpdated', {
            detail: updatedGraphics.map((g) => this.graphicToGeojsonFeature(g)),
            bubbles: true,
            composed: true,
          })
        );
  ...
  async startDrawing(drawGeometryType: string): Promise<void> {
    ...
    const currentLayerType = this.getCurrentLayerGeometryType();
    if (currentLayerType && !DrawEditUtils.validGeometryType(drawGeometryType, currentLayerType)) {
      return;
    }
  ...
  private async updateGeojsonInternal(newGeojson: string | GeoJsonFeatureCollection): Promise<void> {
    ...
    if (this.view && this.graphicsLayer.graphics.length > 0) {
      await this.view.goTo(this.graphicsLayer.graphics.toArray());
    }
  }

  private parseGeojson(input: string | GeoJsonFeatureCollection): GeoJsonFeatureCollection | null {
    try {
      return typeof input === 'string' ? (JSON.parse(input) as GeoJsonFeatureCollection) : input;
    } catch (e) {
  ...
  private buildPopupTemplate(): PopupTemplate {
    ...
    if (typeof this.infoTemplate === 'string') {
      ...
      content = this.infoTemplate.details ?? this.infoTemplate.content ?? '';
    }
  ...
  private resolveLabelConfig(): { expression: string; color: any; size: number } {
    const fallback = {
      ...
      color: this.parseColor(this.labelColor) ?? [0, 0, 0, 255],
      size: this.labelSize ?? 10,
    };
  ...
  disconnectedCallback(): void {
    ...
    this.viewDoubleClickHandle?.remove();
